...
4. Authentication is password-only in the current MVP. SSO and identity-linking flows are intentionally out of scope for now.
5. Group creation is currently allowed for any authenticated enabled user who presents a valid group-creation access code.
6. Archive restore currently records an auditable restore request rather than rehydrating the payload back into the hot database automatically.
